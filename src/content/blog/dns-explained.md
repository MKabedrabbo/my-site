...
Two record types matter most for a project like this. An **A record** maps a domain straight to an IP address. A **CNAME record** maps a domain to *another domain name* instead. That's useful because a host's underlying server IP can change without me ever having to touch my DNS settings, since I'm just pointing at their domain, not a fixed number.
